...
class MainDashboardScreen extends ConsumerStatefulWidget {
  const MainDashboardScreen({
    ...
    this.showOrdersInitially = false,
  });

  final Widget ordersPage;
  final bool showOrdersInitially;
...
class _MainDashboardScreenState extends ConsumerState<MainDashboardScreen> {
  ...
  void didUpdateWidget(covariant MainDashboardScreen oldWidget) {
    ...
    if (oldWidget.showOrdersInitially != widget.showOrdersInitially) {
      _syncInitialTab();
    }
  }
  ...
  void _syncInitialTab() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      ref.read(dashboardViewModelProvider.notifier).onTabChange(
            widget.showOrdersInitially ? AppTab.orders : AppTab.dashboard,
          );
    });
  }
